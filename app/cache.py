...
local current_before_version = tonumber(redis.call('HGET', KEYS[1], 'version') or '0')
local db_version = tonumber(ARGV[3])

if db_version >= current_confirmed and db_version >= current_before_version then
  redis.call(
    'HSET', KEYS[1],
    'value', ARGV[1],
    'before_uuid', ARGV[2],
...
def normalize_cache(data: dict[str, str]) -> dict[str, Any]:
    ...
    for field in ("version", "confirmed_version", "before_written_at_ms"):
